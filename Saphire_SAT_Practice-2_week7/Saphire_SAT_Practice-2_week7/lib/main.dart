...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _startIndex = _loopBase * _cardsData.length;
    _cards = PageController(viewportFraction: _viewport, initialPage: _startIndex);
...
class _Dots extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final pageVal = controller.hasClients && controller.page != null ? controller.page! : startIndex.toDouble();
    ...
    return Row(
      ...
      children: List.generate(count, (i) {
        final diff = (p - i).abs();
        final wrap = math.min(diff, count - diff);
...
class _FlatTopHoneycombPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double stepX = 1.5 * r;
    final double stepY = h;
  ...
  void _drawFlatTopHex(Canvas canvas, double cx, double cy, double r, Paint p) {
    final path = Path();
    for (int k = 0; k < 6; k++) {
      final double angle = (math.pi / 180.0) * (60.0 * k);
